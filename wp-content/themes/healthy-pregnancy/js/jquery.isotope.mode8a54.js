...
var ilightbox_settings = {
  ...
  keepAspectRatio:
    cmsmasters_isotope_mode.ilightbox_aspect_ratio == "1" ? true : false,
  mobileOptimizer:
    cmsmasters_isotope_mode.ilightbox_mobile_optimizer == "1" ? true : false,
  maxScale: Number(cmsmasters_isotope_mode.ilightbox_max_scale),
  minScale: Number(cmsmasters_isotope_mode.ilightbox_min_scale),
  innerToolbar:
    cmsmasters_isotope_mode.ilightbox_inner_toolbar == "1" ? true : false,
  smartRecognition:
    cmsmasters_isotope_mode.ilightbox_mobile_optimizer == "1" ? true : false,
  fullAlone:
    cmsmasters_isotope_mode.ilightbox_fullscreen_one_slide == "1"
      ? true
      : false,
  fullViewPort: cmsmasters_isotope_mode.ilightbox_fullscreen_viewport,
  controls: {
    toolbar:
      cmsmasters_isotope_mode.ilightbox_controls_toolbar == "1" ? true : false,
    arrows:
      cmsmasters_isotope_mode.ilightbox_controls_arrows == "1" ? true : false,
    fullscreen:
      cmsmasters_isotope_mode.ilightbox_controls_fullscreen == "1"
        ? true
        : false,
    thumbnail:
      cmsmasters_isotope_mode.ilightbox_controls_thumbnail == "1"
        ? true
        : false,
    keyboard:
      cmsmasters_isotope_mode.ilightbox_controls_keyboard == "1" ? true : false,
    mousewheel:
      cmsmasters_isotope_mode.ilightbox_controls_mousewheel == "1"
        ? true
        : false,
    swipe:
      cmsmasters_isotope_mode.ilightbox_controls_swipe == "1" ? true : false,
    slideshow:
      cmsmasters_isotope_mode.ilightbox_controls_slideshow == "1"
        ? true
        : false,
  },
  text: {
...
(function (window, factory) {
  ...
  proto._getContainerSize = function () {
    ...
    size.height =
      this.Timeline[this.Timeline.colB > this.Timeline.colA ? "colB" : "colA"];
...
function getNumbColumns(container) {
  var containerWidth = container.width(),
    firstPost = container.find(
      "> article:eq(0), .cmsmasters_gallery_item:eq(0)"
    ),
...
function setPuzzleColumnWidth(container) {
  var containerWidth = container.width(),
    ...
    postStaticHeight = Math.floor(
      (postStaticWidth / 100) * 68.97 /* preloader padding bottom in % */
    ),
    postStaticPadding = Math.floor(
      ((postPaddingLeft + postPaddingRight) / 100) *
        68.97 /* preloader padding bottom in % */
    ),
...
function startBlog(
  ...
  blog.imagesLoaded(function () {
    ...
    if (
      layout === "columns" &&
      (layoutMode === "grid" || layoutMode === "puzzle")
    ) {
      blog.isotope({
        layoutMode: "fitRows",
        resizable: false,
  ...
  blogContainer
    .find(".cmsmasters_post_filter_block .cmsmasters_post_filter_list a")
    .on("click", function () {
  ...
  blogContainer.find(".cmsmasters_post_loader").on("click", function () {
    blogContainer
      ...
      .addClass("cmsmasters_loading cmsmasters_theme_icon_spinner")
      .find(".cmsmasters_wrap_items_loader")
      .slideUp("fast");
      ...
      success: function (data) {
        if (data !== "") {
          ...
          if (data.slice(0, 6) === "finish") {
            ...
            blogContainer
              .find(".cmsmasters_wrap_more_items")
              .removeClass("cmsmasters_loading cmsmasters_theme_icon_spinner")
              .find(".cmsmasters_wrap_items_loader")
              .slideDown("fast");
          }
          ...
          newHTML.imagesLoaded(function () {
            ...
            setTimeout(function () {
              ...
              jQuery(".wp-audio-shortcode, .wp-video-shortcode")
                .filter(":not(.mejs-container)")
                .mediaelementplayer(settings);

              for (var i = 0, il = lightboxVars.length; i < il; i += 1) {
                blogContainer
                  .find('a[rel="' + lightboxVars[i] + '"]')
                  .iLightBox(ilightbox_settings);
              }
            }, 300);
          });
        }
...
function startPortfolio(
  ...
  portfolioContainer
    .find(".cmsmasters_project_filter_block .cmsmasters_project_filter_list a")
    .on("click", function () {
      var filter = jQuery(this),
        filterSelector = filter.attr("data-filter");
  ...
  portfolioContainer
    .find(".cmsmasters_project_sort_block .cmsmasters_project_sort_but")
    .on("click", function () {
  ...
  portfolioContainer
    .find(".cmsmasters_project_loader")
    .on("click", function () {
      portfolioContainer
        .find(".cmsmasters_wrap_more_items")
        .addClass("cmsmasters_loading cmsmasters_theme_icon_spinner")
        ...
        success: function (data) {
          if (data !== "") {
            ...
            if (data.slice(0, 6) === "finish") {
              newHTML = jQuery(
                data
                  .replace(/type-project/g, "type-project shortcode_animated")
                  .slice(6, -1)
              );
              ...
              newHTML = jQuery(
                data.replace(/type-project/g, "type-project shortcode_animated")
              );
            ...
            newHTML.find('a[rel^="ilightbox["]').each(function () {
              if (
                jQuery.inArray(jQuery(this).attr("rel"), lightboxVars) === -1
              ) {
                lightboxVars.push(jQuery(this).attr("rel"));
              }
            });
...
function startGallery(id, gallery_type, gallery_count) {
  ...
  if (gallery_arr.length > 0) {
    galleryContainer
      .find(".cmsmasters_gallery_items_loader")
      .on("click", function () {
        galleryContainer
          .find(".cmsmasters_wrap_more_items")
          .addClass("cmsmasters_loading cmsmasters_theme_icon_spinner")
